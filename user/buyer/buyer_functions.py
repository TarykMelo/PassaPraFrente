    ...
    while True:
        ...
        for produto in produtos:
            texto += (
                ...
                f"Vendedor: {produto[6]}\n"
                f"Telefone: {produto[7]}\n"
                f"{'-' * 40}\n"
            )
...
def filtrar_produto(usuario):
    """
    Essa Função filtra a categoria e vê os produtos disponíveis de lá
...
def ver_meus_pedidos(usuario):
    ...
    for pedido in pedidos:
        texto += (
            f"[bold]#{pedido[0]} — {pedido[2]}[/bold]\n"
            f"Preço: R$ {pedido[3]:.2f}\n"
            f"Vendedor: {pedido[7]}\n"
            f"Telefone: {pedido[8]}\n"
            f"{'-' * 40}\n"
        )
